<?php
// src/transactions/stock_out.php
require_once __DIR__ . '/../../includes/auth_check.php';
requireLogin();
require_once __DIR__ . '/../../includes/db_connection.php';

$products = $pdo->query("SELECT id, name, sku, quantity FROM products ORDER BY name")->fetchAll();
$errors = [];
$success = false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($quantity > $current_qty) {
            $errors[] = 'Not enough stock available. Current stock: ' . $current_qty;
        }
    }
...
<div class="form-page">
    <h1>Stock Out (Remove stock from inventory)</h1>
            ...
            <select name="product_id" required>
                ...
                <?php foreach ($products as $product): ?>
                    <option value="<?= $product['id'] ?>">
                        <?= htmlspecialchars($product['name']) ?> (<?= htmlspecialchars($product['sku']) ?>) - Stock: <?= $product['quantity'] ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
